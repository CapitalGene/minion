...
Worker.prototype.execTask = function (message) {
  ...
  message.getPayload()
    ...
    .catch(errors.Retry, function (retryErr) {
      ...
      // handle retry
      var taskId = message.correlationId;
      var retries = message.headers['x-minion-retries'] || 0;
      var maxRetries = message.headers['x-minion-max-retries'] || self.app.tasks[taskName].maxRetries;
      var countdown = message.headers['x-minion-countdown'] || self.app.tasks[taskName].retryDelay;
      if (retries > maxRetries) {
        return Promise.reject(
          new errors.MaxRetriesExceededError(
            'Can not retry ' + taskName + '[' + taskId + ']')
        );
      }

      return Promise.reject(new Error('NOTIMPLEMENTED RETRY'));
    })
    .catch(errors.Reject, function (rejectErr) {
      debug('execTask', 'catch Reject', rejectErr);
      return Promise.reject(new Error('NOTIMPLEMENTED REJECT'));
      // send noack
    })
    .catch(function (err) {
      // catch all
