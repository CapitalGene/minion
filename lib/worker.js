...
/**
 * And internal task that used for health check on the worker's connection
 */
 ...
 *                                       of queue/task names
 * @param {Boolean} options.handlePingTask whether to handle global minion
 *                                         tasks `minion.ping`
 */
var Worker = function(app, options) {
  ...
  this.handlePingTask = options.handlePingTask;

  if (this.handlePingTask) {
    this.app.task(PingTask);
  }
 ...
 * @param {[type]} options.task [description]
 * @return {Promise}
 */
Worker.prototype.execTask = function(message, options) {
  debug('execTask');
  var self = this;
