...
TaskContext.populateFromHeaders = function (context, headers) {
  ...
  _.forEach(this.headerMap, function (field, key) {
    if (!_.isUndefined(headers[field]) && !_.isNull(headers[field])) {
...
Task.prototype.retry = function (options) {
  debug('retry', options);
  options = options || {};
  var retires = this.getRetries();
  var maxRetries = options.maxRetries || this.getMaxRetries();
...
Task.prototype.stringToIntOr0 = function (value) {
  if (!_.isNull(value) &&
    !_.isUndefined(value)) {
...
Task.prototype.getRetries = function () {
  if (this.context) {
    return this.stringToIntOr0(this.context.retries);
  }
  return 0;
};
...
Task.prototype.getMaxRetries = function() {
  var maxRetries;
  if (this.context) {
    maxRetries = this.stringToIntOr0(this.context.maxRetries);
