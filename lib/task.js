 ...
 * @param {Number} config.prefetchCount the max number of task will be executed at
 *                                      the same time
 */
 ...
 *
 * @param  {object} options
 * @param  {String} options.taskId @required
 * @param  {Number} options.messageTtl `x-message-ttl` in ms
 * @param  {Number} options.queueExpires `x-expires` argument in ms
 *                                       @optional
 *
 * @return {broker.Queue}
 */
 ...
 *
 * > will be called when using app.task()
 *
 * @example
...
Task.prototype.compile = function(app) {
  ...
  newTask.prefetchCount = this.prefetchCount;
  ...
  newTask.prototype.prefetchCount = this.prefetchCount;
